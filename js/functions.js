...
async function runSparlqQuery(url,query,type){
  ...
  try{
    p = new Promise(function(resolve, reject){
      ...
      ////console.log(configFileExpert.file)
      ////console.log(url)
      ...
      if(url.includes("wikidata")){
        ...
        let dataType = configFileExpert.file.filter(obj=>obj.sparqlEndpoint==url)[0]["dataType"]
        if(dataType=="json"){
          settings = { url: queryUrl, async: true   , dataType: 'json',accepts: 
          {
  ...
  if(node!=undefined){
      if((parameters!="")&&(parameters!=null)){
          for (let i = 0; i < parameters.length; ++i) { 
              console.log(parameters[i]["property"])
              console.log(node)
...
/*   var x = d3.event.pageX 
  var y = d3.event.pageY + window.scrollY
  //////console.log(x)
  //////console.log(y)
  ...
  tooltip.style.top = `${(y - (tooltip.clientHeight / 2))}px`; //event.clientY - 150 + "px"
  tooltip.style.left = `${(x + 35)}px`; //event.clientX - 95 + "px"
...
function getCoordinatesBubbles(){
  var xs=[],ys=[],position;
  d3.selectAll("nodeCircleFree")
  var circles=document.getElementsByClassName("nodeCircleBasic");
  for (i=0;i<circles.length;i++){
    //////console.log(circles[i].id)
    position=document.getElementById(circles[i].id.replace("_g","")).getBoundingClientRect()
    xs.push(position.x)
    ys.push(position.y)
  }
  ////////console.log(xs)
