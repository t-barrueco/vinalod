...
/*   function check_filters(node,filters){
  ...
  var vis = this,childrenVisible=false,position;
  vis.visibleNodes=[]
  vis.valuesFilters=[]
  //console.log(filters)
  ...
  for (var i = vis.treeData.length - 1; i >= 0; i--) {
    ...
    if(vis.treeData[i].children!=undefined){
      vis.treeData[i].children.forEach(function(v){
        //console.log(v)
        //console.log(check_filters(v,filters))
        if(check_filters(v,filters)){
          v["hidden"]=true
        }else{
          delete v.hidden
          if(!vis.visibleNodes.includes(v.id)){
            //console.log(v)
            vis.visibleNodes.push(v.id)
          }
          
          //vis.valuesFilters.push(vis.valueFilters[])
/*           position=nodes.indexOf(nodes.filter(function(item) {
            return (item.id == d.id)
          })[0])
          if(position==-1){
          ...
          filters.forEach(function (f){
            //vis.valuesFilters.push({"class":f.class,"property":f.property})
/*             position=vis.valuesFilters.indexOf(vis.valuesFilters.filter(function(item) {
              return ((item.class == f.class)&(item.property == f.class))
            })[0])
            if(position==-1){
              nodes.push(d.id)
            } */
    ...
    if(childrenVisible){
      delete vis.treeData[i].hidden
      if(!vis.visibleNodes.includes(vis.treeData[i].id)){
        //console.log(vis.treeData[i])
        vis.visibleNodes.push(vis.treeData[i].id)
      }
    }else{
      if(check_filters(vis.treeData[i])){
        vis.treeData[i].hidden=true
      }else{
        delete vis.treeData[i].hidden
        if(!vis.visibleNodes.includes(vis.treeData[i].id)){
