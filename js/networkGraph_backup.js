...
/*     .origin(function() { 
    ...
    .on("end", function(){
      ...
      if((vis.dragX!=0)||(vis.dragY!=0)||(vis.zoomScale!=1)){
        console.log("entra")
        var dcx = (window.innerWidth/2-networkGraph.coorX);
        var dcy = (window.innerHeight/2-networkGraph.coorY);
        vis.coorX=((vis.coorX) /vis.zoomScale )
        vis.coorY=((vis.coorY) /vis.zoomScale )
      }
      d3.select('body').style("cursor", "auto");
    });
...
NetworkGraph.prototype.initializeForces = function() {
  ...
  .on("end",function(){
    ...
    if((vis.dblClickId)&&(vis.addingGraph)){
      ...
      //////console.log(d3.select("#"+vis.dblClickId).attr('cx'))
      //console.log("entra")
      var dcx = (window.innerWidth/2-d3.select("#"+vis.dblClickId).data()[0]["x"]);
      var dcy = (window.innerHeight/2-d3.select("#"+vis.dblClickId).data()[0]["y"]);
      //////console.log(document.getElementById(vis.dblClickId).getAttribute("transform"))
      ...
      //////console.log(dcx)
      //////console.log(dcy)
      ////console.log(parseFloat(translate[0]))
      ////console.log(parseFloat(translate[1]))
      ////console.log(dcx-parseFloat(translate[0]))
/*       vis.svg.append("text")
            .attr("x", vis.dragX)
            .attr("y", vis.dragY)
            //.attr("dy", ".35em")
...
/*       vis.svg.append("text")
            ...
            //.attr("dy", ".35em")
            .text("prueba-centro"); */
      //vis.nodeCircle.attr("transform", "translate("+ dcx + "," + dcy  + ")");
      ////console.log(vis.zoomScale)
      //.dragX=dcx+parseFloat(translate[0])
      //vis.dragY=dcy+parseFloat(translate[1])
      //////console.log("translate("+ vis.dragX + "," + vis.dragY  + ")scale("+vis.zoomScale+")")
      vis.g.transition()
      //.duration(750).attr("transform",  "translate("+ vis.dragX + "," + vis.dragY  + ") scale("+vis.zoomScale+")") 
      .duration(750).attr("transform",  "translate("+ dcx + "," + dcy  + ") scale("+vis.zoomScale+")") 

      //.duration(750).attr("transform",  "translate("+ window.innerWidth/2 + "," + window.innerHeight/2  + ") scale("+vis.zoomScale+")") 
...
NetworkGraph.prototype.enterGraph = function(){
    ...
    drawImageClusterFree()
      ...
      function handleDblClickEvent(mouseX,mouseY){
        ...
        clearTimeout(timer);
        prevent = true;
        //vis.dblClickId=id.replace("_image","")+"_g"
        ////////console.log(mouseX)
        ////////console.log(mouseY)
